...
class PSUtil(Module):
    ...
    def _discover_threads(self):
        ...
        discovery = Discovery(key="psutil.thread.discovery", macros="{#THREAD}", values=threads)
        self._report(discovery)

    def _percpu_usage(self):
        percpu_usage = psutil.cpu_percent(percpu=True)

        for index, usage in enumerate(percpu_usage):
            data = Data(
                items={"usage": usage},
                key="psutil.cpu",
                append_key=f"[{index}]",
    ...
    def _percpu_frequency(self):
        percpu_frequency = psutil.cpu_freq(percpu=True)

        for index, frequency in enumerate(percpu_frequency):
            data = Data(
                items={
